...
3. Set a custom hourly rate for one specific role
4. Save changes
5. Verify rate is used when employee works that role

**Expected Results:**
- [ ] Can set different rate per business role
- [ ] Role-specific rate overrides base rate
- [ ] Correct rate is used in labor cost calculations
- [ ] Falls back to base rate if no role rate set

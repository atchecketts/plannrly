<x-layouts.app title="Departments">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <p class="mt-2 text-sm text-gray-700">A list of all departments in your organization.</p>
        </div>
        @can('create', App\Models\Department::class)
            <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
                <a href="{{ route('departments.create') }}" class="block rounded-md bg-indigo-600 px-3 py-2 text-center text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                    Add department
                </a>
            </div>
        @endcan
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @forelse($departments as $department)
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $department->location->name }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $department->business_roles_count }}</td>

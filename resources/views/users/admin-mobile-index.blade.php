<x-layouts.admin-mobile title="Team" active="team">
    <!-- Stats -->
    ...
    <!-- Team Members List -->
    <div class="mb-4">
        <h2 class="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Team Members</h2>
        <div class="space-y-2" id="users-list">
            @forelse($users as $user)
                <a href="{{ route('users.show', $user) }}"
                   class="user-card block bg-gray-900 rounded-lg border border-gray-800 p-3"
                   data-name="{{ strtolower($user->full_name) }}"
                   data-email="{{ strtolower($user->email) }}">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 {{ $user->is_active ? 'bg-brand-900/50' : 'bg-gray-700' }} rounded-full flex items-center justify-center {{ $user->is_active ? 'text-brand-300' : 'text-gray-500' }} font-medium text-sm shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500 truncate">
                                {{ $primaryRole?->department?->name ?? 'No department' }}
                                @if($primaryRole?->name)
